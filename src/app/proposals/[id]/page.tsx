  ...
  const checkAuth = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setIsNicVerified(data.profile.user?.isNicVerified || false)
      } else {
        setIsAuthenticated(false)
        setIsNicVerified(false)
      }
    } catch (error) {
      setIsAuthenticated(false)
      setIsNicVerified(false)
        ...
        <div className="wedding-card p-8">
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              {isAuthenticated && !isNicVerified && (
                <div className="text-sm text-amber-600 dark:text-amber-400 flex items-center justify-center">
                  <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  Verification required to send interest
                </div>
              )}
            </div>
          </div>
              ...
              {similarProposals.map((similarProposal) => (
                <div key={similarProposal.id} className="wedding-card p-6 hover:scale-105 transition-transform duration-300">
                  ...
                  <button
                    ...
                    className="w-full text-center wedding-button py-2 flex items-center justify-center gap-2"
                  >
