...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      prisma.advert.count({ where: { userId, isPublished: true } }),
      prisma.advert.count({ where: { userId, isPublished: false } }),
      prisma.advert.count({ where: { userId, isPublished: true, isActive: true, OR: [{ expiresAt: null }, { expiresAt: { gt: new Date() } }] } }),
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        adverts: advertCount,
        publishedAdverts: publishedAdvertCount,
        draftAdverts: draftAdvertCount,
